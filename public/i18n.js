/* i18n.js — Milliy Pin interfeys matnlari (o'zbekcha va ruscha).
   Kalitlar nuqta bilan guruhlangan: "nav.home", "order.title" va h.k. */
(function () {
        "use strict";

  const DICT = {
    uz: {
      "orders.fAll": "Hammasi", "orders.fOpen": "Ochiq",
      "orders.repeat": "Qayta buyurtma",
      "orders.emptyFilter": "Bu holatda buyurtma yo'q",
      "err.item_gone": "Bu xizmat endi mavjud emas.",
      "err.already_done": "Bajarilgan buyurtmani bekor qilib bo'lmaydi.",
      "lb.today": "Bugun", "lb.week": "Hafta", "lb.month": "Oy", "lb.all": "Hammasi",
      "lb.orders": "buyurtma", "lb.you": "Siz",
      "profile.notif": "Bildirishnomalar",
      "profile.notifSub": "Buyurtma holati va bonuslar haqida xabar",
      "fav.title": "Sevimlilar",
      "fav.empty": "Sevimli mahsulot yo'q",
      "fav.emptySub": "Mahsulot oynasidagi yurak belgisini bosing — u shu ro'yxatga tushadi.",
      "fav.count": "{p} ta mahsulot",
      "fav.added": "Sevimlilarga qo'shildi",
      "fav.removed": "Sevimlilardan olindi",
      "profile.share": "Ilovani ulashish",
      "profile.shareSub": "Do'stlaringizga havola yuboring",
      "profile.anim": "Animatsiyalar",
      "profile.animSub": "Eski telefonlarda o'chirsangiz ilova tezlashadi",
      "profile.loyalty": "Sodiqlik darajasi",
      "profile.loyaltyHint": "Xarid summangiz oshgan sari daraja ko'tariladi va har buyurtmadan keshbek balansga qaytadi.",
      "profile.fromSpent": "dan boshlab",
      "profile.adminSub": "Buyurtma, to'lov va katalog boshqaruvi",
      "home.reviewsCount": "baho",
      "home.noReviews": "Hozircha sharh yo'q",
      "home.noReviewsSub": "Birinchi buyurtmangizdan keyin baho qoldirishingiz mumkin.",
      "profile.socials": "Ijtimoiy tarmoqlar",
      "profile.faq": "Tez-tez beriladigan savollar",
      "nav.catalog": "O'yinlar",
      "nav.topup": "To'ldirish",
      "home.hi": "Salom",
      "home.guest": "Mehmon",
      "home.quickPromo": "Promokodlar",
      "home.quickHelp": "Yordam",
      "cat.title": "Barcha xizmatlar",
      "cat.search": "O'yin yoki xizmat qidirish",
      "cat.nores": "Hech narsa topilmadi",
      "cat.noresSub": "Boshqa so'z bilan qidirib ko'ring yoki filtrni tozalang.",
      "cat.clear": "Qidiruvni tozalash",
      "cat.telegram": "Telegram",
      "cat.game": "O'yinlar",
      "maint": "Texnik ish",
      "topup.method": "To'lov usuli",
      "topup.mUzcard": "UZCARD",
      "topup.mHumo": "HUMO",
      "topup.mCard": "Kartaga o'tkazma",
      "topup.links": "Havolalar",
      "topup.how": "Qanday to'ldirish?",
      "topup.help": "Texnik yordam",
      "promo.title": "Promokodlar",
      "promo.ph": "PROMOKODNI KIRITING",
      "promo.check": "Tekshirish",
      "promo.avail": "Mavjud promokodlar",
      "promo.take": "Olish",
      "promo.saved": "Promokod saqlandi — buyurtmada avtomatik qo'llanadi",
      "promo.none": "Hozircha ochiq promokod yo'q",
      "promo.sub": "Kodni saqlab qo'ying — buyurtmada o'zi qo'llanadi",
      "promo.hint": "Saqlangan kod har bir buyurtmada avtomatik tekshiriladi. Shartga to'g'ri kelmasa, u shunchaki qo'llanmaydi.",
      "lb.title": "Top donaterlar",
      "lb.sub": "Eng ko'p xarid qilganlar",
      "lb.empty": "Hozircha ro'yxat bo'sh",
      "ref.card": "Do'st taklif qiling",
      "ref.cardSub": "Har xarididan bonus oling",
      "err.maintenance": "Bu xizmat vaqtincha texnik ishda.",
      "topup.s1": "Kartaga aynan yuqoridagi summani o'tkazing — oxirgi raqamlar to'lovingizni tanib olish uchun.",
      "topup.s2": "O'tkazma tugagach quyidagi tugmani bosing.",
      "topup.s3": "Summa tasdiqlangach balansingiz avtomatik to'ladi (5-15 daqiqa).",
      "ref.shareText": "Milliy Pin — Telegram xizmatlari va o'yin donatlari qulay narxda",
      "nav.home": "Bosh", "nav.tg": "Telegram", "nav.games": "O'yinlar",
      "nav.orders": "Buyurtma", "nav.profile": "Profil",

      "home.balance": "Sizning balansingiz",
      "home.topup": "To'ldirish",
      "home.popular": "Ommabop",
      "home.again": "Yana buyurtma qilish",
      "home.reviews": "Mijozlar fikri",
      "home.all": "Barchasi",
      "home.stats": "Bizga ishonishadi",
      "home.statUsers": "foydalanuvchi",
      "home.statOrders": "buyurtma",

      "prod.from": "dan",
      "prod.choose": "Paketni tanlang",
      "prod.promo": "Promokod (ixtiyoriy)",
      "prod.saved": "Saqlangan:",
      "prod.check": "Tekshirish",
      "prod.dupHint": "#{p} buyurtmasi bir daqiqa oldin berilgan. Tasodifan ikki marta bosilgan bo'lsa — bekor qiling; ataylab yana bir paket olmoqchi bo'lsangiz — davom eting.",
      "prod.dupYes": "Ha, yana bitta",
      "prod.help": "Qayerdan olinadi?",
      "prod.idOk": "Format to'g'ri ko'rinadi",
      "prod.badId": "O'yinchi ID faqat raqamlardan iborat bo'ladi (5-15 ta raqam).",
      "prod.badZone": "Format: 12345678 (1234) — ID va qavs ichida server raqami.",
      "prod.badUser": "@username lotin harfi bilan boshlanadi, kamida 5 belgi.",
      "prod.badLink": "Havola https:// yoki t.me/ bilan boshlanishi kerak.",
      "prod.promoApply": "Qo'llash",
      "prod.comment": "Izoh (ixtiyoriy)",
      "prod.commentPh": "Qo'shimcha ma'lumot...",
      "prod.subtotal": "Mahsulot narxi",
      "prod.discount": "Chegirma",
      "prod.total": "Jami to'lov",
      "prod.buy": "Sotib olish",
      "prod.needTopup": "Balansni to'ldirish",
      "prod.notEnough": "Balans yetarli emas",

      "field.username": "Telegram username",
      "field.usernamePh": "@foydalanuvchi",
      "field.playerId": "O'yinchi ID (Player ID)",
      "field.playerIdPh": "Masalan: 5123456789",
      "field.playerZone": "ID va Server / Zone",
      "field.playerZonePh": "Masalan: 12345678 (1234)",
      "field.nickname": "O'yindagi nik",
      "field.nicknamePh": "Nikingizni yozing",
      "field.link": "Havola",
      "field.linkPh": "https://t.me/...",

      "topup.title": "Balansni to'ldirish",
      "topup.amount": "Summa",
      "topup.amountPh": "Masalan: 50000",
      "topup.card": "To'lov kartasi",
      "topup.next": "Davom etish",
      "topup.copied": "Nusxa olindi",
      "topup.exact": "Aynan shu summani o'tkazing",
      "topup.paid": "To'lov qildim",
      "topup.cancel": "Bekor qilish",
      "topup.waiting": "To'lovingiz tekshirilmoqda. Odatda 5-15 daqiqa vaqt oladi.",
      "topup.min": "Eng kam summa",
      "topup.unfinished": "Yakunlanmagan to'lov",
      "topup.inCheck": "To'lov tekshirilmoqda",
      "topup.recent": "Oxirgi to'lovlar",
      "topup.rules": "Faqat shaxsiy kartadan o'tkazma qiling. Chek rasmi kerak emas — summa avtomatik tanib olinadi.",

      "orders.title": "Mening buyurtmalarim",
      "orders.empty": "Hozircha buyurtma yo'q",
      "orders.emptySub": "Birinchi buyurtmangizni bering — bir necha daqiqada bajariladi.",
      "orders.payments": "To'lovlar",
      "orders.noPay": "To'lov tarixi bo'sh",
      "orders.noPaySub": "Balansni to'ldirsangiz, o'tkazmalar shu yerda ko'rinadi.",
      "orders.tab": "Buyurtmalar",
      "orders.search": "Raqam, mahsulot yoki ID bo'yicha qidirish",
      "orders.detail": "Buyurtma tafsiloti",
      "orders.num": "Buyurtma raqami",
      "orders.target": "Ma'lumot",
      "orders.date": "Sana",
      "orders.cashback": "Keshbek",
      "orders.note": "Izoh",
      "orders.reason": "Bekor qilish sababi",
      "orders.cancel": "Buyurtmani bekor qilish",
      "orders.cancelAsk": "Buyurtma bekor qilinsinmi?",
      "orders.cancelHint": "Faqat hali ishga olinmagan buyurtmani bekor qilish mumkin. Pul darhol balansingizga qaytadi.",
      "orders.canceled": "Bekor qilindi, pul balansga qaytdi",
      "orders.askSupport": "Operatorga yozish",
      "orders.toCatalog": "Katalogni ochish",
      "orders.review": "Baholash",
      "orders.reviewTitle": "Buyurtmani baholang",
      "orders.reviewText": "Fikringiz (ixtiyoriy)",
      "orders.send": "Yuborish",
      "orders.thanks": "Fikringiz uchun rahmat!",
      "profile.balance": "Balans",
      "profile.spent": "Sarflangan",
      "profile.orders": "Buyurtma",
      "profile.support": "Qo'llab-quvvatlash",
      "profile.channel": "Rasmiy kanal",
      "profile.reviews": "Sharhlar",
      "profile.lang": "Til",
      "profile.theme": "Ko'rinish",
      "profile.admin": "Admin panel",
      "profile.toNext": "Keyingi darajagacha",

      "ref.title": "Referal dastur",
      "ref.desc": "Do'stingiz havolangiz orqali kirib xarid qilsa, har xaridining {p}% i balansingizga tushadi.",
      "ref.link": "Sizning havolangiz",
      "ref.copy": "Havoladan nusxa olish",
      "ref.share": "Do'stlarga yuborish",
      "ref.invited": "Taklif qilingan",
      "ref.active": "Xarid qilgan",
      "ref.earned": "Ishlangan",
      "ref.off": "Referal dastur vaqtincha o'chirilgan.",

      "st.new": "Yangi", "st.processing": "Bajarilmoqda", "st.done": "Bajarildi", "st.canceled": "Bekor qilindi",
      "st.pending": "Kutilmoqda", "st.confirmed": "Tasdiqlandi", "st.rejected": "Rad etildi", "st.expired": "Muddati o'tdi",

      "err.auth": "Ilovani Telegram orqali oching.",
      "err.expired": "Sessiya eskirdi — ilovani qayta oching.",
      "err.network": "Tarmoq xatosi. Qayta urinib ko'ring.",
      "err.insufficient": "Balansingiz yetarli emas.",
      "err.target": "Ma'lumotni to'liq kiriting.",
      "err.min_topup": "Summa juda kichik.",
      "err.promo_not_found": "Bunday promokod topilmadi.",
      "err.promo_expired": "Promokod muddati tugagan.",
      "err.promo_used_up": "Promokod limiti tugagan.",
      "err.promo_already_used": "Siz bu promokoddan foydalangansiz.",
      "err.promo_min_order": "Promokod uchun buyurtma summasi yetarli emas.",
      "err.too_many_pending": "Sizda tasdiqlanmagan to'lov bor. Avval uni yakunlang.",
      "err.blocked": "Hisobingiz vaqtincha bloklangan.",
      "err.too_fast": "Juda tez-tez so'rov yuborildi. Bir daqiqadan keyin urinib ko'ring.",
      "err.shop_closed": "Do'kon vaqtincha yopiq. Tez orada qaytamiz.",
      "err.duplicate": "Bu buyurtma hozirgina berilgan — ikki marta yozilmasligi uchun to'xtatildi.",
      "err.too_late": "Buyurtma ishga olingan — endi bekor qilib bo'lmaydi. Operatorga yozing.",
      "err.no_bot_token": "Server hali sozlanmagan.",
      "err.server_error": "Server xatosi.",

      "ok.ordered": "Buyurtma qabul qilindi!",
      "ok.saved": "Saqlandi",
      "common.close": "Yopish",
      "common.loading": "Yuklanmoqda...",
      "common.som": "so'm",
      "common.retry": "Qayta urinish",
      "common.yes": "Ha",
      "common.cancel": "Bekor qilish"
    },

    ru: {
      "orders.fAll": "Все", "orders.fOpen": "Открытые",
      "orders.repeat": "Повторить",
      "orders.emptyFilter": "В этом статусе заказов нет",
      "err.item_gone": "Эта услуга больше недоступна.",
      "err.already_done": "Выполненный заказ отменить нельзя.",
      "lb.today": "Сегодня", "lb.week": "Неделя", "lb.month": "Месяц", "lb.all": "Всё время",
      "lb.orders": "заказ.", "lb.you": "Вы",
      "profile.notif": "Уведомления",
      "profile.notifSub": "Сообщать о статусе заказа и бонусах",
      "fav.title": "Избранное",
      "fav.empty": "Избранных товаров нет",
      "fav.emptySub": "Нажмите на сердечко в карточке товара — он попадёт в этот список.",
      "fav.count": "{p} товаров",
      "fav.added": "Добавлено в избранное",
      "fav.removed": "Убрано из избранного",
      "profile.share": "Поделиться приложением",
      "profile.shareSub": "Отправьте ссылку друзьям",
      "profile.anim": "Анимации",
      "profile.animSub": "На старых телефонах отключите — станет быстрее",
      "profile.loyalty": "Уровень лояльности",
      "profile.loyaltyHint": "Чем больше сумма покупок, тем выше уровень и больше кешбэк с каждого заказа.",
      "profile.fromSpent": "от",
      "profile.adminSub": "Заказы, платежи и каталог",
      "home.reviewsCount": "оценок",
      "home.noReviews": "Отзывов пока нет",
      "home.noReviewsSub": "После первого заказа вы сможете оставить оценку.",
      "profile.socials": "Мы в соцсетях",
      "profile.faq": "Частые вопросы",
      "nav.catalog": "Игры",
      "nav.topup": "Пополнить",
      "home.hi": "Привет",
      "home.guest": "Гость",
      "home.quickPromo": "Промокоды",
      "home.quickHelp": "Помощь",
      "cat.title": "Все услуги",
      "cat.search": "Поиск игры или услуги",
      "cat.nores": "Ничего не найдено",
      "cat.noresSub": "Попробуйте другое слово или очистите фильтр.",
      "cat.clear": "Очистить поиск",
      "cat.telegram": "Telegram",
      "cat.game": "Игры",
      "maint": "Тех. работы",
      "topup.method": "Способ оплаты",
      "topup.mUzcard": "UZCARD",
      "topup.mHumo": "HUMO",
      "topup.mCard": "Перевод на карту",
      "topup.links": "Ссылки",
      "topup.how": "Как пополнить?",
      "topup.help": "Техподдержка",
      "promo.title": "Промокоды",
      "promo.ph": "ВВЕДИТЕ ПРОМОКОД",
      "promo.check": "Проверить",
      "promo.avail": "Доступные промокоды",
      "promo.take": "Взять",
      "promo.saved": "Промокод сохранён — применится в заказе автоматически",
      "promo.none": "Пока нет открытых промокодов",
      "promo.sub": "Сохраните код — он применится в заказе сам",
      "promo.hint": "Сохранённый код проверяется в каждом заказе. Если условия не подходят, он просто не применяется.",
      "lb.title": "Топ донатеров",
      "lb.sub": "Больше всех покупок",
      "lb.empty": "Список пока пуст",
      "ref.card": "Пригласите друга",
      "ref.cardSub": "Бонус с каждой покупки",
      "err.maintenance": "Услуга временно на техобслуживании.",
      "topup.s1": "Переведите на карту ровно указанную сумму — последние цифры нужны для распознавания платежа.",
      "topup.s2": "После перевода нажмите кнопку ниже.",
      "topup.s3": "Как только сумма подтвердится, баланс пополнится автоматически (5-15 минут).",
      "ref.shareText": "Milliy Pin — услуги Telegram и донат в игры по выгодным ценам",
      "nav.home": "Главная", "nav.tg": "Telegram", "nav.games": "Игры",
      "nav.orders": "Заказы", "nav.profile": "Профиль",

      "home.balance": "Ваш баланс",
      "home.topup": "Пополнить",
      "home.popular": "Популярное",
      "home.again": "Заказать снова",
      "home.reviews": "Отзывы клиентов",
      "home.all": "Все",
      "home.stats": "Нам доверяют",
      "home.statUsers": "пользователей",
      "home.statOrders": "заказов",

      "prod.from": "от",
      "prod.choose": "Выберите пакет",
      "prod.promo": "Промокод (необязательно)",
      "prod.saved": "Сохранённые:",
      "prod.check": "Проверить",
      "prod.dupHint": "Заказ #{p} оформлен минуту назад. Если нажали дважды случайно — отмените; если хотите ещё один пакет — продолжайте.",
      "prod.dupYes": "Да, ещё один",
      "prod.help": "Где взять?",
      "prod.idOk": "Формат выглядит верно",
      "prod.badId": "ID игрока состоит только из цифр (5–15 знаков).",
      "prod.badZone": "Формат: 12345678 (1234) — ID и номер сервера в скобках.",
      "prod.badUser": "@username начинается с латинской буквы, минимум 5 символов.",
      "prod.badLink": "Ссылка должна начинаться с https:// или t.me/",
      "prod.promoApply": "Применить",
      "prod.comment": "Комментарий (необязательно)",
      "prod.commentPh": "Дополнительная информация...",
      "prod.subtotal": "Стоимость",
      "prod.discount": "Скидка",
      "prod.total": "Итого",
      "prod.buy": "Купить",
      "prod.needTopup": "Пополнить баланс",
      "prod.notEnough": "Недостаточно средств",

      "field.username": "Telegram username",
      "field.usernamePh": "@пользователь",
      "field.playerId": "ID игрока (Player ID)",
      "field.playerIdPh": "Например: 5123456789",
      "field.playerZone": "ID и Сервер / Zone",
      "field.playerZonePh": "Например: 12345678 (1234)",
      "field.nickname": "Игровой ник",
      "field.nicknamePh": "Укажите ваш ник",
      "field.link": "Ссылка",
      "field.linkPh": "https://t.me/...",

      "topup.title": "Пополнение баланса",
      "topup.amount": "Сумма",
      "topup.amountPh": "Например: 50000",
      "topup.card": "Карта для оплаты",
      "topup.next": "Продолжить",
      "topup.copied": "Скопировано",
      "topup.exact": "Переведите ровно эту сумму",
      "topup.paid": "Я оплатил",
      "topup.cancel": "Отменить",
      "topup.waiting": "Платёж проверяется. Обычно это занимает 5-15 минут.",
      "topup.min": "Минимальная сумма",
      "topup.unfinished": "Незавершённый платёж",
      "topup.inCheck": "Платёж проверяется",
      "topup.recent": "Последние платежи",
      "topup.rules": "Переводите только с личной карты. Чек не нужен — сумма распознаётся автоматически.",

      "orders.title": "Мои заказы",
      "orders.empty": "Заказов пока нет",
      "orders.emptySub": "Сделайте первый заказ — он выполнится за несколько минут.",
      "orders.payments": "Платежи",
      "orders.noPay": "История платежей пуста",
      "orders.noPaySub": "Пополните баланс — переводы появятся здесь.",
      "orders.tab": "Заказы",
      "orders.search": "Поиск по номеру, товару или ID",
      "orders.detail": "Детали заказа",
      "orders.num": "Номер заказа",
      "orders.target": "Данные",
      "orders.date": "Дата",
      "orders.cashback": "Кэшбэк",
      "orders.note": "Комментарий",
      "orders.reason": "Причина отмены",
      "orders.cancel": "Отменить заказ",
      "orders.cancelAsk": "Отменить заказ?",
      "orders.cancelHint": "Отменить можно только заказ, который ещё не взят в работу. Деньги сразу вернутся на баланс.",
      "orders.canceled": "Отменено, деньги вернулись на баланс",
      "orders.askSupport": "Написать оператору",
      "orders.toCatalog": "Открыть каталог",
      "orders.review": "Оценить",
      "orders.reviewTitle": "Оцените заказ",
      "orders.reviewText": "Ваш отзыв (необязательно)",
      "orders.send": "Отправить",
      "orders.thanks": "Спасибо за отзыв!",
      "profile.balance": "Баланс",
      "profile.spent": "Потрачено",
      "profile.orders": "Заказы",
      "profile.support": "Поддержка",
      "profile.channel": "Официальный канал",
      "profile.reviews": "Отзывы",
      "profile.lang": "Язык",
      "profile.theme": "Тема",
      "profile.admin": "Админ-панель",
      "profile.toNext": "До следующего уровня",

      "ref.title": "Реферальная программа",
      "ref.desc": "Если друг зайдёт по вашей ссылке и что-то купит, {p}% с каждой покупки поступит на ваш баланс.",
      "ref.link": "Ваша ссылка",
      "ref.copy": "Скопировать ссылку",
      "ref.share": "Отправить друзьям",
      "ref.invited": "Приглашено",
      "ref.active": "С покупками",
      "ref.earned": "Заработано",
      "ref.off": "Реферальная программа временно отключена.",

      "st.new": "Новый", "st.processing": "Выполняется", "st.done": "Выполнен", "st.canceled": "Отменён",
      "st.pending": "Ожидает", "st.confirmed": "Подтверждён", "st.rejected": "Отклонён", "st.expired": "Истёк",

      "err.auth": "Откройте приложение через Telegram.",
      "err.expired": "Сессия устарела — откройте приложение заново.",
      "err.network": "Ошибка сети. Попробуйте ещё раз.",
      "err.insufficient": "Недостаточно средств на балансе.",
      "err.target": "Заполните данные полностью.",
      "err.min_topup": "Сумма слишком мала.",
      "err.promo_not_found": "Такой промокод не найден.",
      "err.promo_expired": "Срок промокода истёк.",
      "err.promo_used_up": "Лимит промокода исчерпан.",
      "err.promo_already_used": "Вы уже использовали этот промокод.",
      "err.promo_min_order": "Сумма заказа мала для этого промокода.",
      "err.too_many_pending": "У вас есть неподтверждённый платёж. Сначала завершите его.",
      "err.blocked": "Ваш аккаунт временно заблокирован.",
      "err.too_fast": "Слишком много запросов. Попробуйте через минуту.",
      "err.shop_closed": "Магазин временно закрыт. Скоро вернёмся.",
      "err.duplicate": "Такой заказ только что оформлен — повтор остановлен.",
      "err.too_late": "Заказ уже взят в работу — отменить нельзя. Напишите оператору.",
      "err.no_bot_token": "Сервер ещё не настроен.",
      "err.server_error": "Ошибка сервера.",

      "ok.ordered": "Заказ принят!",
      "ok.saved": "Сохранено",
      "common.close": "Закрыть",
      "common.loading": "Загрузка...",
      "common.som": "сум",
      "common.retry": "Повторить",
      "common.yes": "Да",
      "common.cancel": "Отмена"
    }
  };

  let lang ="uz";
  try { lang = localStorage.getItem("mp_lang") ||"uz"; } catch (e) {}
  if (!DICT[lang]) lang ="uz";

  window.I18N = {
    get lang() { return lang; },
    set(l) {
      if (!DICT[l]) return;
      lang = l;
      try { localStorage.setItem("mp_lang", l); } catch (e) {}
      document.documentElement.lang = l;
    },
    // t("ref.desc", {p: 3}) — {p} kabi o'rniga qo'yish qo'llab-quvvatlanadi
    t(key, vars) {
      let s = (DICT[lang] && DICT[lang][key]) || (DICT.uz[key]) || key;
      if (vars) Object.keys(vars).forEach(k => { s = s.split("{" + k +"}").join(vars[k]); });
      return s;
    },
    // Ko'p tilli maydon: {uz:"...", ru:"..."} yoki oddiy satr
    pick(v) {
      if (v == null) return"";
      if (typeof v ==="string") return v;
      return v[lang] || v.uz || v.ru ||"";
    },
    langs: ["uz", "ru"]
  };
})();
